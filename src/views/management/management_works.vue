  ...
  <div class="box">
    <div class="center">
      ...
      <div class="form">
        <!-- <p>作品</p> -->
        <div class="nav">
          <router-link to="/management_works" style="color: #fff; background-color: #60cdf8;">作品</router-link>
...
export default {
  data() {
    return {
      ...
      editlabel: '',
      edittechnology: '',
      selectedId: null // 用于存储当前选中的作品的 ID
    }
  },
  methods: {
    // 获取当前登录用户
    ...
    async submit () {
      ...
      if (!newWorks.id || !newWorks.title || !newWorks.label || !newWorks.technology) {
        alert('请填写作品信息！')
        return
      }

      try {
        const res = await axios.post('http://localhost:3000/works', newWorks)
        if (res.status === 201) {
          alert('添加作品成功！')
          // 重新刷新页面
          location.reload()
    ...
    async del (id) {
      if (!id) return
      const confirmed = confirm('确定要删除该作品吗？')
      if (confirmed) {
        try {
          const apiUrl = `http://localhost:3000/works/${id}`
    ...
    edit(item) {
      this.isshow = true,
      this.selectedId = item.id // 存储当前要修改的作品的 ID
    ...
    async editsubmit() {
      ...
      try {
        ...
        const apiUrl = `http://localhost:3000/works/${this.selectedId}` // 替换为你的更新 API
...
.food .worklist ul li button:hover{
  cursor: pointer;
  background-color: rgb(240, 205, 241);
  border: none;
}
